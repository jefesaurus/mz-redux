...
int lookupdev()
{
  ...
  while (index)
  {
    ...
    if (index->addresses)
    {
      ...
      while(pcap_addr)
      {
        if (pcap_addr->addr && (pcap_addr->addr->sa_family==AF_INET))
        {
              ...
              IPADDRSIZE))
          {
            ...
            if (verbose)
            {
              fprintf(stderr," mz: device %s got assigned %s ", index->name, ipaddress);
            }
            ...
            if (strncmp(ipaddress, "127", 3)==0)
            {
              ...
              device_list[i].phy=0;
              get_if_ip(index->name, device_list[i].ip);
              get_if_ip(index->name, device_list[i].ip_mops);
              get_if_mac(index->name, device_list[i].mac);
              get_if_mac(index->name, device_list[i].mac_mops);

              //quirksmode way of determining whether the MAC address exists...
              //not sure if I can get it out of PCAP
              int j=0;
              while (j<6 && device_list[i].mac[j] ==0) j++;
              if (j<5) device_list[i].phy=1;
              else device_list[i].phy=0;
              i++;
            }
      ...
      if(!hasIP)
      {
        if(verbose) fprintf(stderr, " mz: Found device %s without IP\n", index->name);
        strncpy(device_list[i].dev, index->name, 9);
        strncpy(device_list[i].ip_str, "n/a", IPADDRSIZE);
        memset(device_list[i].ip,0,4);
        memset(device_list[i].ip_mops,0,4);
        get_if_mac(index->name, device_list[i].mac);
        get_if_mac(index->name, device_list[i].mac_mops);
        int j=0;
        while (j<6 && device_list[i].mac[j] ==0) j++;
...
//
int get_if_ip (char *ifname, u_int8_t *ip)
{
  int fd;
  struct ifreq ifr;
...
// Caller must provide an unsigned char ip[4], mac[6]
//
int get_if_mac (char *ifname, u_int8_t *mac)
{
  int fd;
  struct ifreq ifr;
  int i;
